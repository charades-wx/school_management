...
public class ActivitiController {
    ...
    @Autowired
    private ActivitiService as;

    //1.查询流程定义和部署信息
    @RequestMapping("/bushu/getdeploys")
    public String chaxun(ModelMap map){
        //查询部署信息
        List<Deployment> dlist=as.getdeplist();
        //查询流程定义信息
        List<ProcessDefinition> prolist=as.getprolist();
        map.put("dlist",dlist);
    ...
    public String up(MultipartFile depfile, String name){
        try {
            as.add(depfile.getInputStream(),name);
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public void liuchengtu(String depid, String imagename, HttpServletResponse response){
        ...
        try {
            //根据部署id和图片名称，查看文件流
            InputStream inputStream=as.findimage(depid,imagename);
            //将文件保存到本地
    ...
    public String qingjia(int leaveid, HttpSession session){//请假条的id值
        ...
        as.qingjia(leaveid,userTb.getUserName());
